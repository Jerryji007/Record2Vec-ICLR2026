...
class HiridDataset(Dataset):
    def __init__(self, data_path, y_path, split="train", train_percentile_mask="all", seed=42, perm=False):
        ...
        all_x = data["all_x"]               # shape: (N, T, D)
        all_x_mask = data["all_x_mask"]     # shape: (N, T, D) or similar
        timeseries_y = data["ts_y"]         # shape: (N, T2, D) or similar
        timeseries_y_mask = data["ts_y_mask"]
        # all_x = data["all_x_aligned"]               # shape: (N, T, D)
        # all_x_mask = data["all_x_mask_aligned"]     # shape: (N, T, D) or similar
        # timeseries_y = data["ts_y_aligned"]         # shape: (N, T2, D) or similar
        # timeseries_y_mask = data["ts_y_mask_aligned"]

        # -------- Load labels / masks --------
        data = np.load(y_path)
        mortality_y = data["mort_y"]                      # (N,)
        etimes_y = data["los_y"]                          # (N,)    (kept for clarity)
        time_to_discharge_y = data["dis_time_y"]          # (N,)
        lab_labels_y = data["lab_labels"]                 # (N, ?)
        feature_labels_y = data["feature_labels"]         # (N, ?)
        gender_y = (data["gender_y"] == "M").astype(np.int64)  # (N,)
        age_y = data["age_y"]                             # (N,)

        threshold_20_mask = data["th_20_mask"].astype(bool)
        threshold_50_mask = data["th_50_mask"].astype(bool)
        threshold_100_mask = data["th_100_mask"].astype(bool)
        train_mask = data["train_mask"].astype(bool)
        validation_mask = data["validation_mask"].astype(bool)
        test_mask = data["test_mask"].astype(bool)

        # -------- Split selection --------
        split_mask = {"train": train_mask, "val": validation_mask, "test": test_mask}[split]
        # ensure no overlap among splits
        assert np.sum(train_mask & validation_mask & test_mask) == 0

        # -------- Percentile mask selection (for training subset) --------
        threshold_mask = {
        ...
        # -------- Split filtering for the actual dataset --------
        if split == "train":
        ...
        # X: input time series
        self.X  = masked_x
        # y1: future time series
        self.y1 = masked_y
        # m1: mask for y1, 1 indicates observed
        self.m1 = masked_ym
        # y2: binary label (mortality)
        self.y2 = masked_mor
        # y3: regression label (normalized time-to-discharge)
        ...
        dt = time.time() - t1
        print(f"Dataset load time {dt:.3f}s")
        print(f"[Scalers] age_mean={self.age_mean:.4f}, age_std={self.age_std:.4f}, "
              f"et_mean={self.et_mean:.4f}, et_std={self.et_std:.4f}")
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.tensor(self.X[real_idx]),                           # float (as stored)
            torch.tensor(self.y1[real_idx]),                           # float (as stored)
            torch.tensor(self.m1[real_idx]),                           # 0/1 mask (as stored)
            torch.tensor(self.y2[real_idx], dtype=torch.long),         # mortality
            torch.tensor(self.y3[real_idx], dtype=torch.float32),      # normalized etimes
            torch.tensor(self.y4[real_idx], dtype=torch.int64),        # lab labels
            torch.tensor(self.y5[real_idx], dtype=torch.int64),        # feature labels
            torch.tensor(self.y6[real_idx], dtype=torch.int64),        # gender
            torch.tensor(self.y7[real_idx], dtype=torch.float32),      # normalized age
        )


class HiridLLMDataset(Dataset):
    def __init__(
        self,
        rep_datapath,
        data_path,
        ...
        if perm:
            rng = np.random.default_rng(2025)
            self.D_perm = rng.permutation(self.T2)
            self.y1 = self.y1[:, :, self.D_perm]
            self.m1 = self.m1[:, :, self.D_perm]
...
class HiridTSDEDataset(Dataset):
    """
