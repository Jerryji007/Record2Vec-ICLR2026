...
class PpicuLLMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.rep == "sbt":
            x = torch.tensor(self.X[real_idx]).float()
        elif self.rep == "sbf":
            x = torch.tensor(self.X[real_idx]).transpose(0, 1).float()
        elif self.rep == "all":
            x = torch.tensor(self.X[real_idx]).float()
        else:
            x = torch.tensor(self.X[real_idx]).float()
...
class PpicuTSDEDataset(Dataset):
    """
